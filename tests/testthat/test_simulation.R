...
test_that("Sequences are correct length and number", {
  expect_length(simulate_sequences("AAA", number_sequences = 10), 10)
  expect_setequal(nchar(simulate_sequences("AAA", sequence_length = 50)), 50)
})

test_that("Sequences are only ACTG", {
  expect_true(grepl("^[ACTG]+$",paste0(simulate_sequences(motif="ACGT"), collapse = "")))
})
...
test_that("At least one motif is inserted", {
  expect_error(simulate_sequences(motif="GGG",enrichment = 0.014, number_sequences = 100))
})
